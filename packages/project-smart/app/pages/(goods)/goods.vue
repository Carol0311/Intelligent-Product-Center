...
const { data: pageData } = await useAsyncData(
  ...
  async () => {
    ...
    //获取页面组件数据
    const pageComponents = await getPageDetail({ pageId: 'goods', baseURL: config.public.productApiKey })
    ...
    //获取页面字段数据
    const pageData = await loadPageData({ pageId: 'goods', id: currentPage.value?.id, baseURL: config.public.productApiKey })
